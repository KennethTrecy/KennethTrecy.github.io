...
<ArticlePost {pageMeta}>
	<StructuredSection
		...
		<SimpleText itemprop="about">
			<strong itemprop="keywords">Abstraction</strong> is a process to see a thing as a one rather than as the sum of its parts. It can be rules or equations that are applicable to most situations. In software development, abstractions are used to simplify the process which are common to other tasks. In other words, they are <strong itemprop="keywords">reusable</strong>. They take in different levels which this page is all about.
		</SimpleText>
	...
	<StructuredSection id={levels[0].id}>
		...
		<SimpleText>
			In addition, most applications are only usable in this level. They are relatively easy to install as they can be standalone or in compressed portable format. The downside is that there is a <strong itemprop="keywords">limited customizability</strong> which are usually offered through <em>settings</em> or <em>preferences</em> option. It is expected that the users of these applications are anyone who have <strong itemprop="keywords">basic computer literacy</strong>.
		</SimpleText>
		<SimpleText>
	...
	<StructuredSection id={levels[1].id}>
		...
		<SimpleText>
			<strong itemprop="mainEntity">This level of abstraction allows the creation of different package-level software programs.</strong> Software programs at this level can be managed using environment variables (which are declared on the system, <code>.env</code> files, or other contexts) or command line arguments. Users who want to customize their software should know how to <strong itemprop="keywords">run</strong> or <strong itemprop="keywords">compile</strong> software.
		</SimpleText>
		<SimpleText>
	...
	<StructuredSection id={levels[2].id}>
		...
		<SimpleText>
			Should the user want to customize the program, knowledge in <strong itemprop="keywords">variable declaration</strong> (depending on the programming language used) is a must. They may also need to learn about enumerations or any different data types like boolean and integer.
		</SimpleText>
		<SimpleText>
